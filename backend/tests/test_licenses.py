...
def _mint_campus(max_seats=None):
    return repo.create_campus_license(
        domain_lock="campus.edu",
        admin_emails=["it@campus.edu"],
        created_by_uid="admin",
        max_seats=max_seats,
    )


def test_activate_campus_requires_domain_match(store):
    store._data["users"] = {
...
def test_activate_campus_creates_seat_and_locks_device(store):
    store._data["users"] = {
        "u1": {"email": "student@campus.edu", "access_status": "APPROVED", "plan": "demo"},
    }
    minted = _mint_campus()
    license_id = minted["license"]["id"]
    err, cfg = repo.activate_license("u1", "student@campus.edu", "dev-1", minted["key"])
    assert err == ""
    assert cfg["plan"] == "professional"
    assert cfg["licenseKind"] == "campus"
    ...
    err2, cfg2 = repo.activate_license("u1", "student@campus.edu", "dev-1", minted["key"])
    assert err2 == ""
    assert cfg2["plan"] == "professional"
    assert len(repo.list_campus_seats(license_id)) == 1
    assert store._data["licenses"][license_id]["seatsUsed"] == 1


def test_activate_campus_device_mismatch_once_seat_is_locked(store):
    store._data["users"] = {
        "u1": {"email": "student@campus.edu", "access_status": "APPROVED", "plan": "demo"},
    }
    minted = _mint_campus()
    repo.activate_license("u1", "student@campus.edu", "dev-1", minted["key"])
    err, cfg = repo.activate_license("u1", "student@campus.edu", "dev-2", minted["key"])
    assert err == "license_device_mismatch"
    assert cfg is None


def test_activate_campus_seats_exhausted_returns_409(store):
    store._data["users"] = {
        "u1": {"email": "a@campus.edu", "access_status": "APPROVED", "plan": "demo"},
        "u2": {"email": "b@campus.edu", "access_status": "APPROVED", "plan": "demo"},
    }
    minted = _mint_campus(max_seats=1)
    err1, _ = repo.activate_license("u1", "a@campus.edu", "dev-1", minted["key"])
    assert err1 == ""
    err2, cfg2 = repo.activate_license("u2", "b@campus.edu", "dev-2", minted["key"])
    assert err2 == "license_seats_exhausted"
    assert cfg2 is None


@pytest.mark.asyncio
async def test_activate_campus_seats_exhausted_over_http_is_409(client, monkeypatch):
    ...
    monkeypatch.setattr(deps, "_DEV_USER", {**deps._DEV_USER, "email": "a@campus.edu"})
    store = fake_firestore.install(monkeypatch)
    monkeypatch.setattr(repo.notify, "access_request", lambda *a, **k: None)
    store._data["users"] = {
        "dev-user": {"email": "a@campus.edu", "access_status": "APPROVED", "plan": "demo"},
        "u2": {"email": "b@campus.edu", "access_status": "APPROVED", "plan": "demo"},
    }
    minted = _mint_campus(max_seats=1)
    repo.activate_license("u2", "b@campus.edu", "dev-2", minted["key"])
    resp = await client.post(
        "/v1/licenses/activate",
        json={"key": minted["key"]},
        headers={"X-Device-Id": "dev-device"},
...
def test_revoke_whole_campus_license_drops_every_seat_and_frees_slots(store):
    store._data["users"] = {
        "u1": {"email": "a@campus.edu", "access_status": "APPROVED", "plan": "demo"},
        "u2": {"email": "b@campus.edu", "access_status": "APPROVED", "plan": "demo"},
    }
    minted = _mint_campus()
    license_id = minted["license"]["id"]
    repo.activate_license("u1", "a@campus.edu", "dev-1", minted["key"])
    repo.activate_license("u2", "b@campus.edu", "dev-2", minted["key"])
    ...
    for seat in repo.list_campus_seats(license_id):
        assert seat["status"] == "revoked"


def test_revoke_single_campus_seat_frees_only_that_slot(store):
    store._data["users"] = {
        "u1": {"email": "a@campus.edu", "access_status": "APPROVED", "plan": "demo"},
        "u2": {"email": "b@campus.edu", "access_status": "APPROVED", "plan": "demo"},
    }
    ...
    assert repo.revoke_campus_seat(license_id, "u1") is True
    assert store._data["users"]["u1"]["plan"] == "demo"
    ...
    store._data["users"]["u3"] = {
        "email": "c@campus.edu", "access_status": "APPROVED", "plan": "demo",
    }
...
def test_disable_seat_drops_to_demo_without_freeing_slot(store):
    ...
    store._data["users"]["u2"] = {
        "email": "b@campus.edu", "access_status": "APPROVED", "plan": "demo",
    }
    err, cfg = repo.activate_license("u2", "b@campus.edu", "dev-2", minted["key"])
...
def test_clear_seat_device_lock_lets_seat_holder_rebind(store):
    ...
    minted = _mint_campus()
    license_id = minted["license"]["id"]
    repo.activate_license("u1", "a@campus.edu", "old-phone", minted["key"])
    # Without clearing, a new device on the same seat is rejected.
    err, _ = repo.activate_license("u1", "a@campus.edu", "new-phone", minted["key"])
    assert err == "license_device_mismatch"

    assert repo.clear_seat_device_lock(license_id, "u1") is True
    err2, cfg2 = repo.activate_license("u1", "a@campus.edu", "new-phone", minted["key"])
    assert err2 == ""
    assert cfg2["plan"] == "professional"


def test_activation_is_in_place_session_data_untouched(store):
    """Activating a license (individual or campus) must never migrate or touch
    ...
    minted = _mint_campus()
    err, cfg = repo.activate_license("u1", "a@campus.edu", "dev-1", minted["key"])
    assert err == ""
    assert cfg["plan"] == "professional"
...
def test_downgrade_preserves_data_blocks_creation_then_reactivation_restores(store, monkeypatch):
    ...
    err2, cfg2 = repo.activate_license("u1", "a@campus.edu", "dev-1", minted["key"])
    assert err2 == ""
    assert cfg2["plan"] == "professional"
...
def test_device_lock_is_revalidated_on_every_authed_call_not_just_at_activation(store):
    ...
    repo.revoke_campus_seat(license_id, "u1")
...
@pytest.mark.asyncio
async def test_admin_mint_campus_license_via_http(client, monkeypatch):
    fake_firestore.install(monkeypatch)
    monkeypatch.setattr(repo.notify, "access_request", lambda *a, **k: None)
    resp = await client.post(
        "/v1/admin/licenses",
        json={
            "kind": "campus",
            "domainLock": "campus.edu",
            "adminEmails": ["it@campus.edu"],
            "maxSeats": 50,
        },
    )
    ...
    body = resp.json()
    assert body["key"].startswith("SEMP-")
    assert body["license"]["kind"] == "campus"
    assert body["license"]["domainLock"] == "campus.edu"
    assert body["license"]["maxSeats"] == 50
    assert "emailLock" in body["license"] and body["license"]["emailLock"] == ""


@pytest.mark.asyncio
async def test_admin_mint_campus_license_requires_domain_and_admin_emails(client, monkeypatch):
    fake_firestore.install(monkeypatch)
    monkeypatch.setattr(repo.notify, "access_request", lambda *a, **k: None)
    resp = await client.post("/v1/admin/licenses", json={"kind": "campus"})
    assert resp.status_code == 422


@pytest.mark.asyncio
async def test_campus_seat_patch_clear_device_lock_over_http(client, monkeypatch):
    ...
    monkeypatch.setattr(deps, "_DEV_USER", {**deps._DEV_USER, "email": "it@campus.edu"})
    store = fake_firestore.install(monkeypatch)
    monkeypatch.setattr(repo.notify, "access_request", lambda *a, **k: None)
    store._data["users"] = {
        "dev-user": {"email": "it@campus.edu", "access_status": "APPROVED", "plan": "demo"},
        "student": {"email": "a@campus.edu", "access_status": "APPROVED", "plan": "demo"},
    }
    minted = _mint_campus()
    license_id = minted["license"]["id"]
    repo.activate_license("student", "a@campus.edu", "old-dev", minted["key"])

    resp = await client.patch(
        f"/v1/campus/licenses/{license_id}/seats/student",
...
@pytest.mark.asyncio
async def test_campus_seat_revoke_over_http(client, monkeypatch):
    from app import deps
    ...
    resp = await client.delete(f"/v1/campus/licenses/{license_id}/seats/student")
    assert resp.status_code == 200, resp.text
