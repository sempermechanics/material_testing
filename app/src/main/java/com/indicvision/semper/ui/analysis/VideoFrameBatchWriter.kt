 ...
 * fallback — ends here, so the reference frame, the file naming, the frame
 * times and the commit are decided once and all three produce identical
 * batches.
 */
internal object VideoFrameBatchWriter {
     ...
     * reference, the rest are the deformed batch, each timed by [offsetMsAt]
     * (ms after [startMs]). Null when a frame fails to decode or nothing
     * deformed came out.
     */
    suspend fun write(
        context: Context,
        ...
        for (i in 0 until count) {
            ...
            if (i == 0) {
                ...
                defTimesMs[f.absolutePath] = offsetMsAt(i)
            }
