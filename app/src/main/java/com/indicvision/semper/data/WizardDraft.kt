 ...
 * the frame list and the machine load log as imported. The staged frames themselves stay in
 ...
 * [discard].
 */
@Suppress("TooManyFunctions") // one write and one read per draft part
class WizardDraft(private val dir: File) {
    ...
    /** The load log's text as read from the document; null when none is imported. */
    @WorkerThread
    fun writeLoadLog(text: String?) = write(LOAD_LOG, text?.toByteArray())
    ...
    @WorkerThread
    fun readLoadLog(): String? = read(LOAD_LOG)?.toString(Charsets.UTF_8)
    ...
    companion object {
        ...
        private const val LOAD_LOG = "loads.csv"
