    ...
    cv::Mat bytesToMat(JNIEnv *env, jbyteArray bytes, int expectedWidth = 0, int expectedHeight = 0) {
        ...
        if (expectedWidth > 0 && expectedHeight > 0) {
            if (len == expectedWidth * expectedHeight * 4) {
                // RGBA_8888 Raw Image (from live camera feed)
    ...
    JNIEXPORT jint JNICALL Java_com_rafad_indicvisiondic_IndicVisionNativeLib_computeFullFieldDirect(
            ...
            jboolean applyGaussianBlur, jboolean useNlvcStrain, jboolean use6x6Interpolator, jobject outputBuffer, jobject callbackObj, jfloatArray out_metrics) {
    ...
    #pragma omp parallel for schedule(static) num_threads(safe_cores)
        ...
        if (execute_path_c) {
            ...
            if (!akaze_ref_pts.empty()) {
                float grid_cx = rectX + (gridW / 2.f) * step;
                float grid_cy = rectY + (gridH / 2.f) * step;
                float min_dist = 1e9f;
                ...
                if (best_akaze_idx >= 0) {
                    best_grid_x = std::max(0, std::min(gridW - 1, (int)std::round((akaze_ref_pts[best_akaze_idx].x - rectX) / step)));
                    best_grid_y = std::max(0, std::min(gridH - 1, (int)std::round((akaze_ref_pts[best_akaze_idx].y - rectY) / step)));
                }
            }
            ...
            IndicVision::SubsetPrecomputer::precompute_subset_fast(seed_subset, *g_refImg, realX, realY, subsetSize, hessian_pool[flat_idx]);

            if (seed_subset.is_initialized) {
                IndicVision::OptimizationEngine seed_engine;
                seed_engine.use_6x6_interpolator = use6x6Interpolator;
    ...
    #pragma omp parallel num_threads(safe_cores)
            {
                ...
                local_engine.use_6x6_interpolator = use6x6Interpolator;
    ...
    #pragma omp for schedule(dynamic, 32)
                for (int idx = 0; idx < gridW * gridH; ++idx) {
                    ...
                    if (local_subset.is_initialized) {
                        ...
                        float ghost_fraction = (float)res.invalid_ref_pixels / (float)(subsetSize * subsetSize);
        ...
        // ==========================================
        {
            ...
            prewarm_engine.use_6x6_interpolator = use6x6Interpolator;
