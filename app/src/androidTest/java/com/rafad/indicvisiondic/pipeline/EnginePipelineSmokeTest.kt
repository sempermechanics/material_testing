...
class EnginePipelineSmokeTest {
    ...
    private fun solve(refBytes: ByteArray, defBytes: ByteArray): SolveResult {
        val dims = IndicVisionNativeLib.getImageDimensions(refBytes)
        assertTrue("PNG decode failed in native imgcodecs", dims[0] == W && dims[1] == H)

        IndicVisionNativeLib.initializeReference(refBytes, null, W, H, false)
        ...
        val validPoints = IndicVisionNativeLib.computeFullFieldDirect(
