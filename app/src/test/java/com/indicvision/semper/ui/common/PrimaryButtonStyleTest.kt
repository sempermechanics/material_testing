 ...
 * button and the Cta style (the tap editor's Save, the wizard's Next and Run)
...
class PrimaryButtonStyleTest {
    ...
    fun `a Cta button greys out while disabled`() {
        val root = LayoutInflater.from(themed).inflate(R.layout.activity_beam_edge_tap, null)
        val save = root.findViewById<MaterialButton>(R.id.btnBeamSave)
