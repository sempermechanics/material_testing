...
| Change import / video extraction | `FrameImportHelper`, `VideoFrameExtractor` (three rungs: `AviVideoDecoder` → `HardwareVideoDecoder` → `MediaMetadataRetriever`; all write through `VideoFrameBatchWriter`) |
