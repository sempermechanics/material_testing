...
| Change the stress–strain curve or where it appears | `report/StressStrain.kt` (pure: `Model` per test — axial `P/A`, flexural `3PL/2bh²`, torsional `16T/πd³` with γ = 2·Exy — strain in mε, signed). Viewer ⓘ: `ui/viewer/ViewerStressStrainHelper.kt` (built on first sheet open, cached in `ResultViewerViewModel`). CSV: `AnalysisCsvWriter.mechanicalSuffixColumns`. PDF: `ReportData.mechanical` (cover) and `PdfReportGenerator.generateBatch(stressStrain = …)` (closing page); `ShareCenter.stressStrainPage` renders the plot off screen |
...
| Change the bending / torsion dimensions | `data/SpecimenGeometry.kt` (one field per dimension, `toArray` order is the Intent extra), the matching `StressStrain.Model`, and `ui/analysis/SpecimenGeometryFields.kt` + the `row_specimen_dimension` includes in `wizard_load_card.xml` |
