...
| Change how load-log rows meet frames | `data/MachineLoadMapper.kt` (one-to-one → drop-first → nearest time → resample; the time match takes `logStartS`). Wizard: `ui/analysis/AnalysisLoadCard.kt`, the log-start offset row `LoadSyncRow.kt` (`row_load_sync.xml`) |
