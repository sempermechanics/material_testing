...
| Change import / video extraction | `FrameImportHelper`, `VideoFrameExtractor` (AVI goes to `AviVideoDecoder`; everything else to `MediaMetadataRetriever`). Which instants get sampled is `VideoSampling` — the sheet's estimate and both extraction paths share it, so they cannot disagree |
