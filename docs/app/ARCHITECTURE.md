...
| Change the stress–strain curve or where it appears | `report/StressStrain.kt` (pure: `Model` per test — axial `P/A`, flexural `3PL/2bh²` — strain in mε, signed). Viewer ⓘ: `ui/viewer/ViewerStressStrainHelper.kt` (built on first sheet open, cached in `ResultViewerViewModel`). CSV: `AnalysisCsvWriter.mechanicalSuffixColumns`. PDF: `ReportData.mechanical` (cover) and `PdfReportGenerator.generateBatch(stressStrain = …)` (closing page); `ShareCenter.stressStrainPage` renders the plot off screen |
