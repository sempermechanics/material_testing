...
| TD-47 | Security | `.gitleaks.toml` allowlists every `local.properties` (:14), two paths that no longer exist (:15-16) and four whole commits (:18-23) | 1 | 3 | 1 | **20** | **Partly fixed** (CI/ops PR): the `local.properties` and dead website-sync path rules are gone; `SupabaseManager.kt` path and the four commits stay until a local full-history `gitleaks detect` proves them unneeded |
