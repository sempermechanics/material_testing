...
| TD-62 | Misc | `StudioOverlayView` suppresses `DrawAllocation` over real `RectF` allocations in `onDraw` (:497, :500); `Thread.sleep` in the `RetryOnTransient` interceptor; `create_session` drops the next-page token (`routers/sessions.py:401,469`); `TIMEOUT_US` declared twice; `reconcile_institution_seats` reads one user per seat serially | 1 | 1 | 1 | **10** | **Partly fixed** (backend quick-wins PR): `create_session` returns `nextPageToken` (one page is the whole manifest while `MAX_FILES_PER_SESSION` ≤ `_LIST_SOFT_LIMIT`, now a test), and `reconcile_institution_seats` reads holders with one `get_all`. The app items (`StudioOverlayView`, `RetryOnTransient` sleep, `TIMEOUT_US`) are in the app-reuse PR |
