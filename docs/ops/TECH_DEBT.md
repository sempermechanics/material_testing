...
| TD-59 | Hygiene | Eight remote branches merged into `main` (`git branch -r --merged origin/main`, 2026-09-24): `beta-v0.1`, `beta-v1.2.0`, `damodar`, `video-decoding`, `chore/semper-names`, `docs/perf-142-deployed`, `fix/appcheck-real-phones`, `fix/rate-limit-before-nonce`. `fix/capture-dic-good-practice-gates` is **not** merged: 16 commits (tip `b4be524`) that no tag or other branch holds, and the `archive/*` tags exist only locally | 1 | 1 | 1 | **10** | Close-out PR: owner confirms each deletion; push an `archive/` tag for the capture branch before deleting it |
