...
| TD-27 | Ops | The API Gateway is never deployed by CI; the runbook ([BACKEND_SETUP_GCP.md](../backend/BACKEND_SETUP_GCP.md) "Redeploying the gateway") also had three faults (unset `FIREBASE_PROJECT_ID`, same-day config-id collision, inverted `grep && exit 1`) | 3 | 3 | 3 | **18** | **Built; the IAM grant is in, the diff step fixed** ([ADR-006](../adr/ADR-006-gateway-deploy-job.md)): `gateway` job in `deploy-backend.yml` (dry-run by default; apply creates, switches, verifies, rolls back) and a rewritten runbook. The first production dispatch (run 35963412251, 2026-09-24, `dry-run`) stopped at "Render the spec and diff it against the live config": `apigateway.apis.get` denied. After the grant, the second (run 35990981043, `dry-run`) read the live config `v202609230845` and then failed decoding its document: GNU `base64 -d` rejected it (`base64: invalid input`); the step now decodes either base64 alphabet, with or without padding, in Python. Closes after one `apply` succeeds |
