...
| TD-71 | Security | The deploy SA `indic-deployer` holds two project-wide roles broader than a deploy needs (IAM policy read 2026-09-24). `roles/storage.admin` is not in the documented grant (`CLOUD_ARCHITECTURE_GCP.md:737-740` lists `run.admin`, `artifactregistry.writer`, `iam.serviceAccountUser`, `cloudbuild.builds.editor`), probably added for the source upload of `deploy-cloudrun` (`source: backend`, `deploy-backend.yml:219`); it also lets CI overwrite or delete the Firestore export bucket. `roles/iam.serviceAccountUser` at project level lets CI act as every SA, `firebase-adminsdk-fbsvc` (token creator) included. Reachable only through a compromised WIF deploy identity | 2 | 3 | 2 | **20** | Open, owner (IAM). Check the backup bucket's soft-delete/retention first; then grant `serviceAccountUser` on `indic-api`, `indic-gw` and the Cloud Build SA only and `storage.admin` on the `run-sources-*` bucket only, prove it with a staging deploy and a production `dry-run`, and only then drop the project-level bindings. Record the grant in `CLOUD_ARCHITECTURE_GCP.md` |
...
| TD-74 | App | The ROI mask's background is always "included": `StudioOverlayMaskEncoder.encode` fills the ALPHA_8 bitmap with `Color.BLACK` (alpha 255) before drawing the crop (`ui/analysis/StudioOverlayMaskEncoder.kt:39`), so only the ROI rect bounds the crop and holes are the only exclusions. Latent: the crop tools offer only rectangle and square (`RoiDrawActivity.kt:190-193`), but the circle / ellipse / freeform branches (`StudioOverlayMaskEncoder.kt:66-75`) would be wrong outside their shape | 1 | 2 | 1 | **15** | Open. Clear to 0 before drawing the crop, or delete the unreachable modes; add a mask test that asserts outside the crop |
...
| TD-76 | Performance | After TD-75, `quickSelect` (`report/VisualizationEngine.kt:139`) still went superlinear on sorted and staircase fields: its median-of-three pivot (ends and middle) stops being representative after the first partition, so the p98 pick touched about n^1.5 elements (~370n, 0.8 s on the JVM at 1.2 M points). A V field in grid order is a staircase. `HotPathMicroBenchmark.valueRanges` measured 14.9 ms against the 1.15 ms the full sort it replaced took (`docs/perf/round2-main-vs-branch.md`); 3c88922e swapped the sort out without a benchmark | 2 | 3 | 1 | **24** | **Fixed**: Tukey's ninther for the pivot (`VisualizationEngine.kt:182`) keeps every shape tried near 4n, and an introselect cap sorts the rest of the range after 2·log2(n) passes, bounding the worst case at O(n log n). The picks stay those of a full sort (400 000 fuzzed picks); `VisualizationEngineTest` bounds sorted/staircase time and covers the fallback. On the API 37 emulator (debug), `valueRanges` went 14.9 → 4.6 ms, ×150 2.24 → 0.74 s, `buildReport` 121 → 89 ms. The benchmark's U/V columns are exact ramps, which a sort finishes in one run-detecting pass; on real session frames quickSelect is 2–4× faster than the sort |
| TD-77 | App | PDFs came out as 87.5 × 123.8 cm pages: `PdfLayoutEngine` (`report/PdfLayoutEngine.kt:23`) and `LabReportPdf` (`report/LabReportPdf.kt:45`) passed their 2480 × 3508 layout size (A4 at 300 dpi) to `PdfDocument.PageInfo`, which takes PDF points (1/72 in). Viewers and fit-to-page printing scale it down, so it looked right; printed at actual size it ran 4.2× A4 each way, and PDF tools reported a non-standard paper size (found on the emulator, 2026-09-24) | 2 | 2 | 1 | **16** | **Fixed**: `report/PdfA4` starts 595 × 842 pt pages and scales the canvas by 595/2480, so both layouts keep their design units; text now reads 8.6–13.9 pt. `PdfReportDeviceTest` and `LabWorkflowDeviceTest` assert every MediaBox is `0 0 595 842` |
| TD-78 | App | The lab-report tables numbered rows 1…n, and nothing tied a row to a photo: bending drops unloaded frames and merges a held load's frames (`BeamDeflection.loadSteps`, `report/BeamDeflection.kt:194`), and tensile skips frames with no load or field (`StressStrain.build`). On the PMMA run the peak was frame 33 in the viewer and row 32 in the PDF (found on the emulator, 2026-09-24) | 2 | 2 | 1 | **16** | **Fixed**: both tables add a **Frame** column with the viewer's 1-based frame (`LabReportFormat.frameCell`), a range such as 4–6 for a held load (`BeamDeflection.Step.lastFrame`); S.No still counts rows, as the handwritten reports do. `LabReportTest` / `LabReportBendingTest` cover a skipped and a held frame |
